...
class MainActivity : AppCompatActivity(), Callback<List<Laptop>> {
    private val tag = this::class.java.canonicalName

    private val binding by lazy { ActivityMainBinding.inflate(layoutInflater) }
    private val adapter by lazy { LaptopAdapter(this) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.items.onItemClickListener = AdapterView.OnItemClickListener { _, _, i, _ ->
            ...
            if (book != null) {
                ...
                intent.putExtra("ep.rest.id", book.item_id)
                startActivity(intent)
            }
        }

        binding.container.setOnRefreshListener { LaptopService.instance.getAll().enqueue(this) }

        LaptopService.instance.getAll().enqueue(this)
    }
